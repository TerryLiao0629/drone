...
exports.errorTracker = (dbName, fieldName, fieldValue) => new Promise((resolve, reject) => {
  const whereObj = {};
  whereObj[fieldName] = fieldValue;
  dbName.find({
    attributes: [fieldName],
    where: whereObj
  }).then((dbResult) => {
    logger.info(`[commonService][errorTracker]: ${JSON.stringify(dbResult)}`);
    resolve(dbResult);
  }).catch((error) => {
    logger.error(`[commonService][errorTracker][Error]: ${error}`);
    reject(error);
  });
});
